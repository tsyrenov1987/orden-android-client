...
fun HomeScreen(vm: HomeViewModel = viewModel()) {
    ...
    // Session timer: tick once a second while connected.
    var nowMs by remember { mutableStateOf(System.currentTimeMillis()) }
    LaunchedEffect(state) {
        while (state == VpnState.Connected) { nowMs = System.currentTimeMillis(); delay(1000) }
    }
        ...
        Column(
            ...
            if (state == VpnState.Connected) {
                ConnectionDetailsCard(
                    location = location,
                    ip = egressIp,
                    uptime = formatUptime(if (connectedSince > 0) nowMs - connectedSince else 0),
                )
            ...
            when {
                info != null && code.isNotBlank() -> CabinetCard(
                    ...
                    onRenew = { openUrl(context, TunnelConfig.botUrl) },
...
@Composable
private fun ConnectionDetailsCard(location: String?, ip: String?, uptime: String) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(20.dp)) {
            Text(
                text = "СОЕДИНЕНИЕ",
